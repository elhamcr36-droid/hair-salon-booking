...
# ปรับปรุง CSS ให้แสดงผลสีดำชัดเจนและ Layout สวยงาม
st.markdown("""
    <style>
...
# ฟังก์ชันดึงข้อมูลแบบ TTL=0 เพื่อป้องกัน Cache ค้าง
...
if st.session_state.page == "Home":
    ...
    st.divider()
    google_maps_url = "https://maps.app.goo.gl/สงขลา"
    st.markdown(f"""
        <div class="contact-section-black">
            <h3>📞 ติดต่อเรา & พิกัดร้าน</h3>
            <p>📱 <b>เบอร์โทรศัพท์:</b> 081-222-2222</p>
            <p>💬 <b>LINE ID:</b> @222salon</p>
            <p>📍 <b>พิกัด:</b> <a style="color:blue; text-decoration:underline;" href="{google_maps_url}" target="_blank">ต.บ่อยาง อ.เมืองสงขลา จ.สงขลา</a></p>
        </div>
    """, unsafe_allow_html=True)
...
# 🔑 หน้าจองคิวและแชท (ลูกค้า)
    ...
    with t1:
        with st.form("b_form"):
            ...
            if st.form_submit_button("ยืนยันการจอง"):
                ...
                # เช็คการจองซ้ำตามเงื่อนไข Strict Validation
                duplicate = df_all[(df_all['date'] == str(b_d)) & (df_all['time'] == b_t) & (df_all['status'] == "รอรับบริการ")]
                if b_d.weekday() == 5: st.error("❌ หยุดทุกวันเสาร์")
                elif not duplicate.empty: st.error("❌ เวลานี้มีผู้จองแล้ว กรุณาเลือกเวลาอื่น")
                else:
    ...
    with t3:
        ...
        with st.form("chat_f", clear_on_submit=True):
            txt = st.text_input("พิมพ์ข้อความที่ต้องการสอบถาม...")
...
# 📊 หน้าแอดมิน
    ...
    with at3:
        ...
        if not df_adm_msg.empty:
            unique_u = df_adm_msg['username'].unique()
            sel_u = st.selectbox("เลือกแชทลูกค้า", unique_u)
            user_msgs = df_adm_msg[df_adm_msg['username'] == sel_u]
            for idx, m in user_msgs.iterrows():
                # แสดงข้อความแชทในรูปแบบ Chat message
                st.chat_message("user").write(m['message'])

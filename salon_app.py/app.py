...
# --- 2. ฟังก์ชันดึงข้อมูลและจัดการชื่อผู้ใช้ ---
    ...
    with t_admin1:
        ...
        if not df_b.empty:
            ...
            for _, row in df_admin.sort_values(['date','time'], ascending=[False, True]).iterrows():
                ...
                with st.expander(f"{status_color} {row['time']} | {row['fullname']}"):
...
# --- 5. หน้าจองคิว & ติดต่อเรา (User) ---
    ...
    with t1:
        # ส่วนจองคิว (Services & Date Input)
        SERVICES_BASE_PRICE = {"ตัดผมชาย": 150, "ตัดผมหญิง": 350, "สระ-ไดร์": 200, "ทำสีผม": 1500}
        svc = st.selectbox("เลือกบริการ", list(SERVICES_BASE_PRICE.keys()))
        d = st.date_input("เลือกวันที่")
        if st.button("ยืนยันการจอง"):
            df_b = get_data("Bookings")
            new_id = str(int(datetime.now().timestamp()))
            new_r = pd.DataFrame([{"id":new_id, "username":st.session_state.username, "service":svc, "date":str(d), "time":"09:00", "price":str(SERVICES_BASE_PRICE[svc]), "status":"รอรับบริการ"}])
    ...
    with t3:
        st.write(f"### 💬 แชทกับแอดมิน (ในนาม: {st.session_state.fullname})")
        df_msg = get_data("Messages")
        ...
        for _, m in my_msgs.iterrows():
            ...
            if m['sender'] == "user":
                with st.popover("📝 จัดการข้อความ"):
                    ...
                    if c_sav.button("💾 บันทึก", key=f"sav_{m['msg_id']}"):
                        df_msg.loc[df_msg['msg_id'] == m['msg_id'], 'text'] = new_txt
                        conn.update(worksheet="Messages", data=df_msg); st.rerun()
                    if c_del.button("🗑️ ลบข้อความ", key=f"msg_del_{m['msg_id']}", type="primary"):
...
# --- 6. หน้า Login & Register ---
    ...
    if st.button("เข้าสู่ระบบ"):
        ...
        if u == "admin222" and p == "222":
            st.session_state.update({'logged_in':True, 'user_role':'admin', 'username':'Admin', 'fullname':'แอดมินร้าน'}); navigate("Admin")
        elif not user.empty:
    ...
    with st.form("reg"):
        ...
        if st.form_submit_button("สมัคร"):
            ...
            conn.update(worksheet="Users", data=pd.concat([df_u, new_u], ignore_index=True)); st.success("สมัครสำเร็จ!"); navigate("Login")

...
if not st.session_state.logged_in:
    ...
    if st.session_state.get('user_role') == 'admin':
        ...
        with m_cols[2]:
            if st.button("✂️ จองคิว/แชท"): navigate("Booking")
    with m_cols[4]:
...
if st.session_state.page == "Home":
    ...
    st.markdown("""
        <div class="contact-section-black">
            ...
            <p>📍 <b>พิกัด:</b> ต.บ่อยาง อ.เมืองสงขลา จ.สงขลา 90000</p>
        </div>
    """, unsafe_allow_html=True)
...
# 📝 สมัครสมาชิก
    ...
    with st.form("reg_form"):
        ...
        if st.form_submit_button("ลงทะเบียน"):
            ...
            elif nf and nu and np:
                new_u = pd.DataFrame([{"phone": nu, "password": np, "fullname": nf, "role": "user"}])
                conn.update(worksheet="Users", data=pd.concat([df_u, new_u], ignore_index=True))
                st.success("✅ สำเร็จ! กรุณาเข้าสู่ระบบ"); time.sleep(1.5); navigate("Login")
...
# 🔑 เข้าสู่ระบบ
    ...
    if st.button("ตกลง", type="primary"):
        if u_in == "admin222" and p_in == "222":
            st.session_state.update({'logged_in': True, 'user_role': 'admin', 'username': u_in, 'fullname': 'แอดมิน'})
            navigate("Admin")
        else:
            df_u = get_data("Users")
    ...
    with t1:
        with st.form("b_form"):
            ...
            if st.form_submit_button("ยืนยัน"):
                ...
                elif not active.empty: st.warning("⚠️ คุณมีคิวที่ยังค้างอยู่")
                else:
            ...
            for _, r in my.iterrows():
                with st.container(border=True):
                    ...
                    if r['status'] == "รอรับบริการ":
                        if st.button("❌ ยกเลิกคิว", key=r['id']):
        ...
        if not df_msg.empty:
            ...
            for _, m in my_m.iterrows():
                ...
                if m.get('admin_reply'):
                    box.markdown(f'<div class="chat-msg admin-msg"><b>ร้าน:</b> {m["admin_reply"]}</div>', unsafe_allow_html=True)
        with st.form("chat_f", clear_on_submit=True):
            txt = st.text_input("พิมพ์ข้อความ...")
            if st.form_submit_button("ส่ง") and txt:
                new_m = pd.DataFrame([{"id": str(uuid.uuid4())[:8], "username": st.session_state.username, "message": txt, "timestamp": datetime.now().strftime("%H:%M"), "admin_reply": ""}])

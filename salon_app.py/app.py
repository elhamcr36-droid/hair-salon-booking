...
st.markdown("""
    <style>
        ...
        [data-testid="stChatMessage"][data-testid^="stChatMessageUser"] {
            ...
            margin-left: auto !important; width: fit-content !important;
        }
        [data-testid="stChatMessage"][data-testid^="stChatMessageAssistant"] {
            background-color: #E4E6EB !important; color: black !important;
            border-radius: 15px 15px 15px 2px !important;
            margin-right: auto !important; width: fit-content !important;
        }
    </style>
    ...
    with st.form("reg"):
        ...
        if st.form_submit_button("ลงทะเบียน"):
            if not nf or not nu or not np: st.error("❌ กรุณากรอกข้อมูลให้ครบ")
                ...
                new_u = pd.DataFrame([{"phone": nu.strip(), "password": hashed_pw, "fullname": nf.strip(), "role": "user"}])
...
# 3. หน้าเข้าสู่ระบบ
    ...
    if st.button("ตกลง", type="primary"):
        ...
        if u_clean == "admin222" and p_in == "222":
            ...
            df_u = get_data("Users")
            user = df_u[(df_u['phone'] == u_clean) & (df_u['password'] == p_hashed)] if not df_u.empty else pd.DataFrame()
            if not user.empty:

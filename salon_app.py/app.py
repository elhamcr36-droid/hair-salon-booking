...
# 📍 ข้อมูลติดต่อ (อ้างอิงจากโค้ดแรกของคุณ)
SHOP_TEL = "0875644928"
FACEBOOK_URL = "https://www.facebook.com/share/18Yv8zGNoG/?mibextid=wwXIfr" 
LINE_URL = "https://line.me/ti/p/yourid" # ใส่ลิงก์ Line ของคุณ
...
st.markdown(f"""
    <style>
        ...
        /* สไตล์ตารางราคา (Fix Text Visibility) */
        .price-card {{
        ...
        .metric-title {{ font-size: 16px; color: #555 !important; font-weight: bold; }}
        .metric-value {{ font-size: 32px; color: #FF4B4B !important; font-weight: 900; }}
    </style>
...
if st.session_state.page == "Home":
    ...
    services = {
        "✂️ ตัดผมชาย (Men's Cut)": "150 - 350", "💇‍♀️ ตัดผมหญิง (Women's Cut)": "350 - 800",
        "🚿 สระ-ไดร์ (Wash & Blow)": "200 - 450", "🎨 ทำสีผม (Hair Color)": "1,500 - 4,500",
        "✨ ยืดผมวอลลุ่ม": "1,000 - 5,500", "🌿 เคราตินบำรุงผม": "1,500 - 3,500"
    }
...
# --- 5. PAGE: VIEW TODAY'S QUEUE (แสดงเฉพาะคิวรอรับบริการ) ---
elif st.session_state.page == "ViewQueues":
    st.subheader("📅 รายการคิวรอรับบริการวันนี้")
    df_b = get_data("Bookings")
    today = datetime.now().strftime("%Y-%m-%d")
    # ✅ กรองเฉพาะวันนี้ และ สถานะต้องเป็น "รอรับบริการ" เท่านั้น
    active_today = df_b[(df_b['date'] == today) & (df_b['status'] == 'รอรับบริการ')]
    
    if active_today.empty:
        st.info("ยังไม่มีคิวจองที่รอรับบริการในวันนี้")
    else:
        st.dataframe(active_today[['time', 'service']].sort_values('time'), use_container_width=True)

# --- 6. PAGE: BOOKING & HISTORY (แสดงเฉพาะคิวที่ยังไม่เสร็จ) ---
    ...
    with t1:
        svc_list = ["ตัดผมชาย", "ตัดผมหญิง", "สระ-ไดร์", "ทำสีผม", "ยืดผมวอลลุ่ม", "เคราติน"]
        svc = st.selectbox("เลือกบริการ", svc_list)
        d = st.date_input("วันที่จอง", min_value=datetime.now().date())
        t = st.selectbox("เวลา", [f"{h:02d}:00" for h in range(10, 20)])
        
        if d.weekday() == 2: st.error("❌ ร้านหยุดทุกวันพุธ")
        elif st.button("✅ ยืนยันการจอง"):
            df_b = get_data("Bookings")
            new_r = pd.DataFrame([{"id":str(int(datetime.now().timestamp())), "username":st.session_state.username, "service":svc, "date":str(d), "time":t, "price":"0", "status":"รอรับบริการ"}])
            conn.update(worksheet="Bookings", data=pd.concat([df_b, new_r])); st.success("จองสำเร็จ!"); st.rerun()

    with t2:
        df_b = get_data("Bookings")
        # ✅ กรอง: ของตัวเอง + เฉพาะที่ยัง "รอรับบริการ"
            ...
            for _, r in my_active.iterrows():
                with st.container():
                    ...
                    if c_btn.button("❌ ยกเลิก", key=f"can_{r['id']}"):
                        conn.update(worksheet="Bookings", data=df_b[df_b['id'] != r['id']])
                        st.rerun()
...
# --- 7. PAGE: ADMIN DASHBOARD ---
    ...
    adm_t1, adm_t2 = st.tabs(["📉 จัดการคิวงาน", "💬 แชทลูกค้า"])
    ...
    with adm_t1:
        ...
        # Dashboard
        c1, c2 = st.columns(2)
        c1.markdown(f"<div class='metric-container'><div class='metric-title'>👥 ลูกค้าวันนี้</div><div class='metric-value'>{len(df_today)}</div></div>", unsafe_allow_html=True)
        c2.markdown(f"<div class='metric-container'><div class='metric-title'>📅 วันที่</div><div class='metric-value'>{today_str}</div></div>", unsafe_allow_html=True)
        
        st.divider()
        # ✅ แอดมินจัดการเฉพาะคิวที่ "รอรับบริการ"
        pending = df_b[df_b['status'] == 'รอรับบริการ']
        if not pending.empty:
            df_adm = pd.merge(pending, df_u[['username', 'fullname', 'phone']], on='username', how='left')
            for _, r in df_adm.sort_values(['date','time']).iterrows():
                with st.expander(f"🔵 {r['time']} | {r['fullname']} - {r['service']}"):
                    st.write(f"📞 โทร: {r['phone']} | วันที่: {r['date']}")
                    if st.button("✅ งานเสร็จสิ้น (คิวจะถูกซ่อน)", key=f"ok_{r['id']}"):
    ...
    with adm_t2:
        df_msg = get_data("Messages")
        if not df_msg.empty:
            target = st.selectbox("เลือกห้องแชท", df_msg['username'].unique())
            st.markdown('<div class="chat-container">', unsafe_allow_html=True)
            for _, m in df_msg[df_msg['username'] == target].sort_values('msg_id').iterrows():
                cls = "user-msg" if m['sender'] == "user" else "admin-msg"
                st.markdown(f'<div class="bubble {cls}">{m["text"]}</div>', unsafe_allow_html=True)
            st.markdown('</div>', unsafe_allow_html=True)
            with st.form("adm_rep", clear_on_submit=True):
                rep = st.text_input("ตอบแชท...")
                if st.form_submit_button("ส่ง"):
